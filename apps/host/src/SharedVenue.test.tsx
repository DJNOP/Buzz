...
import { getVenueLightingSegments } from "./VenueLightingProgress";
...
describe("shared Signal Sprint venue", () => {
  ...
  it("maps every player's score to only that player's lighting segment", () => {
    const segments = getVenueLightingSegments(
      [
        player(1, { score: 0 }),
        player(2, { score: 1 }),
        player(3, { score: 10 }),
    ...
    expect(segments.map(({ state }) => state)).toEqual([
      "broken",
      "partial",
      "nearlyOperational",
      "complete",
